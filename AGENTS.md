...
#### プロンプト例
```
Github Apps に Gemini Code Assistant, GitHub Copilot を追加してPRを自動レビューさせるようにしました。
...
1. **PR 作成**
2. **AI レビュアー対応**（Gemini、Copilot のレビューコメントへの返信・修正がすべて完了）
3. **CI が通ること**（全チェック green）
4. **自動マージ**（PR作成時に `gh pr merge --auto --squash` を実行しておくことで、項目2と3が満たされた時点で自動マージされる）

PR 作成後は自動的に Gemini・Copilot のレビューを待ち、フィードバックへの対応・CI 通過・マージまでをセットで実施する。
