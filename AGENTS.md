...
- **AIレビュアー**: GitHub Apps（Gemini、Copilot等）からのフィードバックに対応
...
1. **PR 作成**
2. **AI レビュアーのコメント確認と返信・修正**（Gemini、Copilot のコメントに対応）
3. **AI レビュアーからの承認コメント待機**（返信後、各レビュアーから承認が明確な返信が来るまで待つ）
   - OK とみなす表現例：「LGTM」「問題ありません」「修正を確認しました」等（感謝のみの返信は承認とみなさない）
   - 2分待っても返信がない場合：再度メンション付きでコメントする（最大2回まで再試行）
   - 計2回の再試行を行っても返信がない場合：そのまま次のステップに進んでよい
4. **CI が通ること**（全チェック green）
5. **マージ**（`gh pr merge <PR番号> --squash` で実行）

※ `gh pr merge --auto` は AI の OK を人間が確認する前に自動マージされる可能性があるため使用しない。AI の OK 確認後に手動でマージすること。

PR 作成後は自動的に Gemini・Copilot のレビューを待ち、返信 → AI からの OK 確認 → CI 通過 → マージまでをセットで実施する。
