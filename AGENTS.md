...
3. **AI レビュアーからの承認コメント待機**（返信後、各レビュアーから肯定的な返信が来るまで待つ）
   - OK とみなす表現例：「LGTM」「問題ありません」「修正を確認しました」「ありがとうございます」等
   - 5分待っても返信がない場合：再度メンション付きでコメントする（最大2回）
   - 2回試みても返信がない場合：そのまま次のステップに進んでよい
4. **CI が通ること**（全チェック green）
5. **マージ**（`gh pr merge --squash` で実行）

※ `gh pr merge --auto` は CI 通過のみでマージされるため使用しない。AI の OK 確認後に手動でマージすること。
